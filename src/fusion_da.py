...
class RectBatchSampler(torch.utils.data.Sampler):
    """BatchSampler compatible with YOLODataset's rect=True mode.

    rect=True sorts images by aspect ratio and precomputes batch_shapes
    indexed by position: items [0..bs-1] letterbox to one shape, [bs..2bs-1]
    to another, etc. A standard shuffled DataLoader breaks this — random
    indices come from different aspect-ratio buckets and torch.stack dies
    on mismatched H/W.

    This sampler treats each contiguous block of `batch_size` indices as a
    fixed batch (preserving in-batch shape coherence) and only shuffles the
    *order of blocks* between epochs. Tail block (size < batch_size) is kept.
    """
    ...
    def __iter__(self):
        order = torch.randperm(self.num_batches).tolist() if self.shuffle else range(self.num_batches)
        for b in order:
