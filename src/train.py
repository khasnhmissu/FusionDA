...
from fusion_da import FDALoss, WeightEMA, PairedMultiDomainDataset, RectBatchSampler
...
def train(opt):
    ...
    # rect=True in the underlying YOLODataset sorts images by aspect ratio
    # and indexes batch_shapes by position. shuffle=True would mix buckets and
    # break torch.stack. RectBatchSampler shuffles whole batches instead.
    paired_loader = DataLoader(
        paired_dataset,
        batch_sampler=RectBatchSampler(len(paired_dataset), opt.batch, shuffle=True),
    ...
    try:
        ...
        if getattr(opt, 'eval_source', False) or baseline_mode:
            source_yaml = getattr(opt, 'source_data', 'configs/data/data_clear.yaml')
            _ = validate_model(
                tag='STUDENT-INIT', state_dict=model_student.state_dict(),
                weights_path=opt.weights, data_yaml=source_yaml, split='test',
                device=device, dataset_label=f'source_real ({source_yaml})',
            )
    ...
    for epoch in range(opt.epochs):
        ...
        for i in pbar:
            ...
            # ══════════════════════════════════════════════════════════
            # BASELINE MODE: only source_real + GT
            # ══════════════════════════════════════════════════════════
            if baseline_mode:
            ...
            with torch.amp.autocast('cuda', enabled=use_amp):
                    ...
                    if epoch % 5 == 0 and i == 0:
                        ...
                        if teacher_map50_gate is not None:
                            ...
                            if teacher_map50_gate > best_teacher_map50:
                                ...
                                # Keep snapshot on CPU — a GPU clone of the full
                                # model doubles weight memory and was a major
                                # contributor to the epoch-10 OOM.
                                best_teacher_state = {
                                    k: v.detach().to('cpu', copy=True)
                                    for k, v in model_teacher.state_dict().items()
                                }
                                LOGGER.info(
                        ...
                        if gate_pause:
                            ...
                            if best_teacher_state is not None and best_teacher_map50 > 0.1:
                                ...
                                # Snapshot lives on CPU — load_state_dict copies
                                # into the GPU module's existing tensors.
                ...
                #   loss_distillation:∂/∂θ via forward #3 (normal)
        ...
        if (epoch + 1) % 5 == 0 or epoch == opt.epochs - 1:
            try:
                ...
                if current_map50 > best_student_map50:
                    ...
                    # Keep snapshot on CPU to avoid a second full-model copy
                    # living on the GPU alongside the live student.
                    best_student_state = {
                        k: v.detach().to('cpu', copy=True)
                        for k, v in model_student.state_dict().items()
                    }
...
def parse_args():
    ...
    parser.add_argument('--source-data', type=str, default='configs/data/data_clear.yaml',
                        help='YAML used for source_real validation when --eval-source is set '
                             '(default: configs/data/data_clear.yaml). Use data_wider_clear.yaml '
                             'for the WIDERFACE experiments.')
...
if __name__ == '__main__':
    ...
    if args.config:
        ...
        cli_args = args  # keep original argparse Namespace
        config = load_config(args.config)
        config = merge_cli_args(config, args)
        args = config_to_namespace(config)
        # Preserve CLI-only flags (e.g. --eval-source, --source-data, --baseline)
        # that config_to_namespace doesn't emit, so they don't get silently dropped.
